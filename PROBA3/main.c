...
int main(int argc, char* argv[]) {
	...
	CvCapture* capture = cvCaptureFromCAM(0); // initialize capture device
	...
	while (1) {
		...
		IplImage* image_binary = cvRetrieveFrame(capture);

		// show images
		cvResizeWindow(wName, gray->width, gray->height);
		cvShowImage(wName, gray);
